...
app.layout = html.Div([
    ...
    html.Div(dcc.Checklist(["Modificar grafo"], ["Modificar grafo"], id="mod-checklist", inline=True), style={'display': 'inline-block', 'marginRight': '1px'}), #Checklist; Si está seleccionada, se pueden agregar aristas clickeando nodos, y se puede borrar aristas clickeándolas.
    html.Div(
...
def undo_func(undo_clicks, graph_dict, graph_dict_counter, undo_state, lines_state, position, weights):
    ...
    if position: #Si está marcada la checklist de fijar grafo.
    ...
    if undo_state: #Si se puede deshacer
        ...
        G = g_dict_to_nx(g_dict) #G es el grafo anterior al que se está accediendo
